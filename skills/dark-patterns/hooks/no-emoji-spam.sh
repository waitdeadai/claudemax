...
if [ "$count" -gt "$THRESHOLD" ]; then
  # Tier 3 (POC): consult the Haiku judge cascade before the regex BLOCK.
  # v1 POC honors WARN / LOG / REDACT (escalation rc=0); a Haiku BLOCK is
  # treated as agreement and falls through to the existing regex BLOCK below.
  # Fails CLOSED — missing lib, missing cmax CLI, or escalation error leaves
  # the regex verdict unchanged.
  HAIKU_JUDGE_LIB="$(dirname "$0")/../lib/haiku-judge.sh"
  if [ -f "$HAIKU_JUDGE_LIB" ]; then
    # shellcheck disable=SC1090
    if source "$HAIKU_JUDGE_LIB" 2>/dev/null && declare -f haiku_judge_escalate >/dev/null 2>&1; then
      if ( haiku_judge_escalate "$INPUT" ); then
        exit 0
      fi
    fi
  fi
