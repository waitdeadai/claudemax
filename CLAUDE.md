...
- `packages/runtime` — `@anthropic-ai/claude-agent-sdk` `query()` wrappers: orchestrator (Mode A subagents), agent-teams (Mode B), /goal driver, verifier, verify-research (decomposed per-claim research verification, 2026-06-09), spec writer, multispec engine, deepresearch, taste, hive, council, agent-factory, overnight, billing.
...
- `packages/cli` — `cmax` binary (commander). New v0.2 commands: doctor, taste, overnight, research, config. `cmax loop` (loop mode, added 2026-06-08): `run` = converge a goal to DONE (Archetype A); `add|tick|ls|status|pause|resume|kill` = standing, scheduled, input-driven loop fleet (Archetype B, the Boris "write loops" pattern) — verify-gated, dedup-ledgered, fleet-budget-guarded. NOT a `/loop` skill (collides with native Claude Code `/loop`, like the retired `/workflow`). Engine: `packages/{core/src/loop.ts (pure decideNext),runtime/src/{loop,loop-ledger,loop-budget,standing-loop}.ts}`. Docs: `docs/LOOP_MODE.md` + `docs/LOOP_MODE_PLAN.md`. `cmax ask` is intentionally NOT loop-by-default. Loop executor default is era-aware since 2026-06-09 (Opus 4.8 pre-split / Sonnet post-split, same `execModelForVariant` rule as ask; `--opusolo`/`--fable` override). Grounding: `cmax memory propose-decision|propose-fact` (append `status=proposed`) + `cmax ground migrate|compile|export`.
...
- `.claude/agents/grounded-worker.md` — the worker grounding contract (mirrored as `GROUNDED_WORKER_CONTRACT` in `packages/runtime/src/prompts.ts`, prepended to every sub-Spec leaf spawn). `.claude/agents/searchoclock-{researcher,validator}.md` + `.claude/commands/searchoclock.md` are committed, de-namespaced copies of the searchoclock plugin's agents/command (the vendored hook dispatches them by bare name; see `docs/searchoclock.md`). `.claude/commands/converge.md` (also installed user-level at `~/.claude/commands/`) and `.claude/commands/e2e.md` are the `/converge` + `/e2e` slash entries wrapping `cmax loop run` (`/e2e` adds an in-session brainstorm phase that sharpens a rough ask into a measurable goal first) — commands don't count against the lean skill catalog.
...
4. **`/verify` and `/spec` and `/architect` always run on Opus.** Never demote them, even with `--cheap` or past 70/90/95% monthly credit. They are also never auto-escalated to Fable — pinned means pinned (explicit `--tier fable` is the only override).
5. **Fable 5 is access-gated: two router configurations, auto-switched on 2026-06-23.** Added 2026-06-09 (launch day), gated same day. `claude-fable-5` is a fourth tier ABOVE Opus ($10/$50 per MTok = 2× Opus; 1M context; 128K output). `resolveFableAccess()` in `packages/core/src/cost.ts` resolves `included` (through 2026-06-22, Fable free on Max) vs `credits` (after — Fable bills usage credits); override either way via `CMAX_FABLE_ACCESS=included|credits`. While `included`: `plan` baselines to Fable, multispec decompose runs on Fable, and `signal.longHorizon` escalates `plan`/`debug-hard` to Fable. While `credits`: ALL Fable defaults auto-demote back to the Opus configuration; only explicit opt-in (`--tier fable`, `cmax loop run --fable`) reaches Fable — until Anthropic folds it back into subscription/credit access, at which point flip `CMAX_FABLE_ACCESS=included`. Always, in both modes: security-domain packets clamp to Opus (Fable's safety classifiers fall back to Opus 4.8 on cybersecurity-shaped requests; SDK/headless requests end the turn instead); execution stays Sonnet/Opus per era; SDK call sites pass the full id `claude-fable-5` (the `fable` alias is CLI-only as of 2026-06-09), never send `thinking:{type:"disabled"}` to it, and use Opus as `fallbackModel`. Sources: anthropic.com/news/claude-fable-5-mythos-5, platform.claude.com choosing-a-model, code.claude.com/docs/en/model-config (accessed 2026-06-09).
6. **Multispec is the default.** Every umbrella auto-runs deepresearch + multispec + parallel /goal + verify. No `--multi` flag. Single-spec mode is an internal engine optimization.
7. **/taste is auto-bootstrap, NOT 10 questions.** Replaces v1's /tastebootstrap. Uses /deepresearch on SOTA at current time. One fallback question only when the repo has no signal.
8. **Two parallelism modes auto-selected.** Mode A (SDK subagents in one query()) for small/short. Mode B (Claude Code Agent Teams with shared task list + worktree isolation) for big multi-day swarms. Override with `--mode {auto|solo|teams}`. Mode B sub-Specs run in a DAG-aware bounded-parallel dispatcher (`packages/runtime/src/agent-teams.ts` post 2026-05-22 fix): independent leaves dispatch concurrently via a `Promise.race(active)` active-set capped by `MAX_PARALLEL_AGENTS` env or `os.cpus().length`; dependency chains serialize; cycle-stuck sub-Specs fail-fast rather than deadlock.
9. **Workers return evidence.** Every packet emits `EVIDENCE:` + `STATUS:` blocks. Dark-patterns hooks block fake aggregator claims.
10. **No comments explaining what.** Code says what. Comments only for non-obvious why.
11. **Plan-aware cost-guard.** 70% → guard (demote non-essential Opus). 90% → danger (aggressive demote). 95% → blocked (`--force` required).
12. **Closeout shape** — when a turn ends partial / blocked / runtime-pending, OPEN the final message with `**Status: partial — <one-line reason>**` (or `Status: blocked` / `Status: runtime-pending` / `Status: paused` / `Status: in-progress` / `Status: unverified`). The `no-vibes.sh` hook recognizes this header in the first 800 chars as a self-declared honest closeout and skips the body scan. Without the header, positive verbs ("done", "ready", "passed", "shipped") anywhere in the message can trigger a block when paired with any failure-shaped phrase. The hook's repair guidance is canonical: `Status: ... / Verification: ... / Next step: ...`. `Status: complete` is INTENTIONALLY not in the allow list — completion claims still need evidence (commands run, tests passed).
13. **`agentcloseout-physics` is the SOTA scorer for closeout hooks.** 19 of 31 dark-pattern hooks delegate to it for ~1ms deterministic scoring (replaces the regex fallback). `setup.sh` installs it via tiered fallback: cargo-binstall → cargo install from git → local clone+build. If missing, hooks still work via bash regex but trigger more false positives.
14. **searchoclock closes the `errors_solutions` loop.** The vendored `searchoclock.sh` (`PostToolUseFailure(Bash)` primary + defensive `PostToolUse(Bash)`, wired via `.claude/hooks/soc.sh`) WRITES validated durable fixes into `errors_solutions`; the grounding layer (`grounded-worker` + `mcp__memory__memory_search`) READS them. Its independent validator is Anthropic-only by hard rule: `SEARCHOCLOCK_VALIDATOR_CROSS_PROVIDER` stays empty; validator = Haiku 4.5, escalate = Sonnet 4.6. The opt-in `PreToolUse(Bash)` preflight stays off by default (`SEARCHOCLOCK_PROACTIVE=0`). `/searchoclock` is a COMMAND, not a catalog skill (doesn't count against the lean skill catalog). See `docs/searchoclock.md`.
15. **Grounding: agents propose, humans bless, compile promotes.** Agents only ever append memory rows with `status='proposed'` (`cmax memory propose-decision|propose-fact`) — they NEVER self-bless. A human flips `blessed: true` in `vault/`; `cmax ground compile` promotes blessed notes → `accepted` rows and compiles `invariant=1` rows into the `GROUNDING:BEGIN … END` managed block of the matching `CLAUDE.md` (the block is written by the compile tool, never hand-edited). The memory MCP is read-only. Staleness is dated via `last_verified_at`/`ttl_days`; the freshness-gate hook surfaces stale invariants as assumptions.
