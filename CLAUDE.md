...
5. **Fable 5 is escalation-only, never a baseline.** Added 2026-06-09 (launch day). `claude-fable-5` is a fourth tier ABOVE Opus ($10/$50 per MTok = 2× Opus; 1M context; 128K output). The router reaches it only via `signal.longHorizon` on `plan`/`debug-hard` (work larger than a single sitting — overnight runs, multi-day converge loops, ambiguous root-cause hunts) or explicit `--tier fable`. Security-domain packets stay on Opus (Fable's safety classifiers fall back to Opus 4.8 on cybersecurity-shaped requests; SDK/headless requests end in a refusal instead). SDK call sites pass the full id `claude-fable-5` (the `fable` alias is CLI-only as of 2026-06-09), never send `thinking:{type:"disabled"}` to it, and use Opus as `fallbackModel`. Billing: included on Max at no extra cost ONLY through 2026-06-22, then bills to usage credits regardless of billing era — `fableOnUsageCredits()` in `packages/core/src/cost.ts`. Sources: anthropic.com/news/claude-fable-5-mythos-5, platform.claude.com choosing-a-model, code.claude.com/docs/en/model-config (accessed 2026-06-09).
